...
public class TreasureChest implements Runnable {
    ...
    private void loadFrames() {
        frames = new BufferedImage[2]; // Number of frames for the treasure chest
        try {
            // Load the sprite sheet containing the treasure chest frames
            BufferedImage chestSheet = ImageIO.read(getClass().getResource("../item/Chest.png"));

            // Loop through each frame and extract it from the sprite sheet
            for (int i = 0; i < frames.length; i++) {
                frames[i] = chestSheet.getSubimage(i * 21, 0, 21, 18); // Get each frame based on its index
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
