...
public class ChickenLeg implements Runnable {
    ...
    private void loadFrames() {
        frames = new BufferedImage[1]; // Adjust the number of frames as needed
        try {
            // Load the sprite sheet containing the chocolate bar frames
            BufferedImage chocolateSheet = ImageIO.read(getClass().getResource("../item/ChickenLeg.png"));

            // Loop through each frame and extract it from the sprite sheet
            for (int i = 0; i < frames.length; i++) {
                frames[i] = chocolateSheet.getSubimage(i * 0, 0, 16, 16); // Get each frame based on its index
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
