...
public class Fireball implements Runnable {
    ...
    public Fireball(int startX, int startY, boolean direction, Character character) {
        ...
        this.damage = 100 + character.getAttackPower();
        this.character = character;
     
        frames = new BufferedImage[4];
        try {
            BufferedImage fireballSpritesheet = ImageIO.read(getClass().getResource("../character/SpriteSheetSpark.png"));
            for (int i = 0; i < 4; i++) {
    ...
    public void draw(Graphics g) {
        if (isVisible) {
            g.drawImage(frames[frameIndex], x, y+10, width, height, null);
